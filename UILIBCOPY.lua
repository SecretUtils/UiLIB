...
return LPH_NO_VIRTUALIZE(function()
		...
		local config_holder
			...
			function library:anchor_popup(frame, anchor, align_right)
				if not (frame and anchor and frame.Parent and anchor.Parent) then return end
				local ap, asz = anchor.AbsolutePosition, anchor.AbsoluteSize
				local psz = frame.AbsoluteSize
				local root = frame.Parent
				local vp = (root and root.AbsoluteSize) or vec2(1920, 1080)
				local gap = 2

				local ins = 0
				local sg = frame:FindFirstAncestorWhichIsA("ScreenGui")
				if sg and sg.IgnoreGuiInset then ins = gui_service:GetGuiInset().Y end
				local ax, ay = ap.X, ap.Y + ins

				local x = align_right and (ax + asz.X - psz.X) or ax
				if psz.X > 0 then x = clamp(x, 0, max(0, vp.X - psz.X)) end

				local below = ay + asz.Y + gap
				if psz.Y <= 0 or below + psz.Y <= vp.Y then
					frame.Position = dim2(0, x, 0, below)
					return
				end
				local above = ay - psz.Y - gap
				if above >= 0 then
					frame.Position = dim2(0, x, 0, above)
					return
				end
				local y = clamp(ay, 0, max(0, vp.Y - psz.Y))
				local right = ax + asz.X + gap
				if right + psz.X <= vp.X then
					frame.Position = dim2(0, right, 0, y)
				else
					frame.Position = dim2(0, max(0, ax - psz.X - gap), 0, y)
				end
			end

			function library:track_popup(frame, anchor, align_right)
				library.tracked_popup = frame and {frame = frame, anchor = anchor, right = align_right} or nil
				if frame then library:anchor_popup(frame, anchor, align_right) end
			end

			library:connection(run.RenderStepped, function()
				local t = library.tracked_popup
				if not t then return end
				if not (t.frame.Parent and t.frame.Visible and t.anchor.Parent) then
					library.tracked_popup = nil
					return
				end
				library:anchor_popup(t.frame, t.anchor, t.right)
			end)
			...
			function library:window(properties)
				...
				function window.set_menu_visibility(bool)
					...
					local configSec, fontSec = column:multi_section({names = {"Configs", "Fonts"}})
						...
						local lastBaseFont = defaultFont
						...
						fontSec:toggle({name = "Bold", flag = "lib_font_bold", default = false,
							...
							function library:apply_saved_font()
								fontBold = flags["lib_font_bold"] == true
								...
								if PRESET_FONTS[sel] then
									...
									applyLibFont((ok and f) or defaultFont)
									return
								end
								local filename = sel:match("[^\\/]+$") or sel
								local path = fontPathMap[filename]
								if not path then applyLibFont(defaultFont); return end
								local ok, f = pcall(function()
			...
			function library:esp_preview(properties)
				...
				local items = cfg.items; do
					...
					local drag_start_y, drag_start_pitch = 0, 0

					items.viewportframe.Active = true
					...
					library:connection(uis.InputChanged, function(input)
						if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement
							...
							cfg.pitch = math.clamp(drag_start_pitch + (input.Position.Y - drag_start_y) * 0.6, -85, 85)
							cfg.target_pitch = cfg.pitch
						end
					end)
					...
					local function update_preview_overlays()
						...
						if o.chams_box then
							...
							if cham_on then
								local fc = flags[cflag("Chams_Fill_Color")]
						...
						if o.chams then
							if not B and enabled and flags[cflag("Chams")] then
								...
								o.chams.FillTransparency    = flags[cflag("Chams_Fill")]       and (1 - (flags[cflag("Chams_Fill_Opacity")]    or 100) / 100) or 1
						...
						if enabled and flags[cflag("esp_mat_enabled")] then
							local ok, mat = pcall(function() return Enum.Material[flags[cflag("esp_mat_choice")] or "Neon"] end)
						...
						if o.head_dot then
							...
							if sp then
								...
								local c = flags[cflag("HeadDot_Color")]; if c and c.Color then o.head_dot.BackgroundColor3 = c.Color end
						...
						if o.skel_box then
							...
							if skel_on then
								local col = flags[cflag("Skeleton_Color")]
						...
						if o.skel_lines then
							if not B and enabled and flags[cflag("Skeleton")] then
								local col = flags[cflag("Skeleton_Color")]; col = (col and col.Color) or rgb(255, 255, 255)
						...
						if o.holder then
							local box_on = (B and B.box == true) or (not B and flags[cflag("Boxes")] == true)
							...
							if box_on then
								local c1 = (flags[cflag("Box_Color")] and flags[cflag("Box_Color")].Color) or rgb(255, 255, 255)
								local c2 = (flags[cflag("Box_Color_2")] and flags[cflag("Box_Color_2")].Color) or rgb(0, 0, 0)
								local f1 = (flags[cflag("Box_Fill_Color")] and flags[cflag("Box_Fill_Color")].Color) or rgb(255, 255, 255)
								local f2 = (flags[cflag("Box_Fill_Color_2")] and flags[cflag("Box_Fill_Color_2")].Color) or rgb(0, 0, 0)
								local fillOp = math.clamp((flags[cflag("box_fill_opacity")] or 15) / 100, 0, 1)
								local rot = (flags[cflag("Box_Animated")] == true) and ((tick() * 120) % 360) or 90
								...
								if o.box_color_grad then
									...
									o.box_color.Color = rgb(255, 255, 255)
								end
							else
				...
				cfg.change_health = function()
					...
					local lowf  = flags[cflag("Health_Low")]
					local highf = flags[cflag("Health_High")]
					local color = rgb(255, 255, 255)
					if lowf and lowf.Color and highf and highf.Color then
						color = lowf.Color:Lerp(highf.Color, multiplier)
					end
				...
				function cfg.refresh_elements( )
					objects.holder.Parent = cmaster() and items.viewportframe or library.cache
					...
					for flag,object in temp do
						local v = flags[cflag(flag)]
						if v ~= nil then
							if type(object) == "table" then
								if v.Color then object[1].TextColor3 = v.Color end
							elseif cfg._chip_set and cfg._chip_set[object] then
							else
								object.Parent = v and objects[ "holder" ] or library.cache
							end
						end
					end

					local bt = flags[cflag("Box_Type")]
					local is_corner = (bt == nil) or (bt == "Corner")

					local box_on
					if cfg._show then box_on = cfg._show.box == true else box_on = flags[cflag("Boxes")] == true end

					if box_on then
						objects[ "corners" ].ZIndex = 2
						objects[ "box_handler" ].ZIndex = 2
						if is_corner then
					...
					do
						local bc  = flags[cflag("Box_Color")]
						local bc2 = flags[cflag("Box_Color_2")]
						objects[ "box_color" ].Thickness =
							math.clamp(tonumber(flags[cflag("box_stroke_thickness")]) or 1, 1, 5)
						if bc and bc.Color then
							objects[ "box_color" ].Color = bc.Color
							local top_col = bc.Color
							local bot_col = (bc2 and bc2.Color) or rgb(0, 0, 0)
							for _, corner in objects[ "corners" ]:GetChildren() do
								local inner = corner:FindFirstChildOfClass("Frame")
								if inner then
									inner.BackgroundColor3 =
										(corner.Position.Y.Scale >= 1) and bot_col or top_col
								end
							end
						end
					end
					...
					do
						local hp_on
						if cfg._show then hp_on = cfg._show.healthbar == true else hp_on = flags[cflag("Healthbar")] == true end
				...
				local _esp_ok, _esp_err = pcall(function()
					...
					local function make_side(anchor, pos, halign, valign)
						...
						library:create("UIListLayout", {
							...
							SortOrder = Enum.SortOrder.LayoutOrder; Padding = dim(0, 2);
						})
						return c
					end
					...
					containers.left   = make_side(vec2(1, 0), dim2(0, -3, 0, -1), Enum.HorizontalAlignment.Right,  Enum.VerticalAlignment.Top)
					containers.right  = make_side(vec2(0, 0), dim2(1, 3, 0, -1),  Enum.HorizontalAlignment.Left,   Enum.VerticalAlignment.Top)
					...
					local function refresh_side_offsets()
						local ts = math.clamp(math.floor(tonumber(flags[cflag("esp_text_size")]) or 13), 8, 24)
						for _, c in ipairs(CHIPS) do
							local o = objects[c.key]
							if o and o:IsA("TextLabel") and o.TextSize ~= ts then o.TextSize = ts end
						end
						for _, s in ipairs({"top", "bottom", "left", "right"}) do
							if containers[s] then
								local off = 3 + bar_depth(s)
								if s == "top" then
									containers[s].Position = dim2(0.5, 0, 0, -off)
								elseif s == "bottom" then
									containers[s].Position = dim2(0.5, 0, 1, off)
								elseif s == "left" then
									containers[s].Position = dim2(0, -off, 0, -1)
								else
									containers[s].Position = dim2(1, off, 0, -1)
								end
							end
						end
					end
					...
					pcall(function() tray_layout.Wraps = true end)
					local palette_chips = {}
					local chip_active   = {}
					...
					local function make_drag_ghost(label)
						...
						library:apply_theme(drag_ghost, "accent", "BackgroundColor3")
					...
					local function save_layout()
						...
						flags[lflag()] = table.concat(parts, ",")
					end
					...
					local function refresh_palette()
						for _, c in ipairs(CHIPS) do
							...
							if chip then
								if not ctx_allows(c.key) then
									chip.Visible = false
								elseif GLOBAL_KEYS[c.key] or BAR_KEYS[c.key] then
									chip.Visible = true
					...
					local function place(key, side, before_key)
						if BAR_KEYS[key] then
							...
							layout_bars_on_side(side)
							if old and old ~= side then layout_bars_on_side(old) end
						...
						if before_key and placed[before_key] == side then
							order[key] = (order[before_key] or 1) - 0.5
						else
							local mx = 0
					...
					local function unplace(key)
						if BAR_KEYS[key] then
							...
							if old then layout_bars_on_side(old) end
							refresh_side_offsets()
					...
					library:connection(uis.InputEnded, function(input)
						...
						if BAR_KEYS[key] then
							...
							elseif hover_side then place(key, hover_side)
							else place(key) end
					...
					for _, c in ipairs(CHIPS) do
						local okc, errc = pcall(function()
							...
							library:apply_theme(chip, "accent", "BackgroundColor3")
					...
					local function open_settings(key)
						if not settings_panels[key] then return end
					...
					library:connection(uis.InputBegan, function(input)
						if not cfg._builder_on then return end
					...
					local function apply_from_string(str)
						...
						for entry in tostring(str or ""):gmatch("[^,]+") do
							...
							if k and not ctx_allows(k) then
								k = nil
							end
							if k and s and GLOBAL_KEYS[k] then
								placed[k] = "on"; cfg._show[k] = true; any = true
							elseif k and s and BAR_KEYS[k] then
								placed[k] = s; cfg._show[k] = true; apply_bar(k, s); any = true
						...
						if not any and str == nil then
							for _, c in ipairs(CHIPS) do
								local side = DEFAULT_SIDE[c.key]
								if side and objects[c.key] and ctx_allows(c.key) then objects[c.key].Parent = containers[side]; placed[c.key] = side end
							end
						end
					...
					local tabbar_layout = library:create("UIListLayout", {
						Parent = tabbar; FillDirection = Enum.FillDirection.Horizontal;
						HorizontalAlignment = Enum.HorizontalAlignment.Left;
						VerticalAlignment = Enum.VerticalAlignment.Center;
						SortOrder = Enum.SortOrder.LayoutOrder; Padding = dim(0, 4);
					})
					pcall(function() tabbar_layout.Wraps = true end)
					local tab_btns = {}
					local function make_tab(id, label, ord)
						local b = library:create("TextButton", {
							...
							BorderSizePixel = 0; AutomaticSize = Enum.AutomaticSize.X; Size = dim2(0, 0, 0, 16); ZIndex = 7;
			...
			function library:embedded_box(options)
				...
				local well = library:create("Frame", {
					Parent = bg,
					Name = "",
					LayoutOrder = 2,
					...
					BorderColor3 = rgb(0, 0, 0),
					BackgroundColor3 = themes.preset.outline
				}) library:apply_theme(well, "outline", "BackgroundColor3")

				local well_inline = library:create("Frame", {
					Parent = well,
					Name = "",
					Position = dim2(0, 1, 0, 1),
					Size = dim2(1, -2, 1, -2),
					...
					BorderColor3 = rgb(0, 0, 0),
					BackgroundColor3 = themes.preset.inline
				}) library:apply_theme(well_inline, "inline", "BackgroundColor3")

				local well_body = library:create("Frame", {
					Parent = well_inline,
					Name = "",
					Position = dim2(0, 1, 0, 1),
					Size = dim2(1, -2, 1, -2),
				...
				local vp, vp_world, vp_model, vp_base, vp_sg
				local vp_rot, vp_drag, vp_dragx, vp_dragrot = 0, false, 0, 0
				local vp_pitch, vp_dragy, vp_dragpitch = 0, 0, 0

				local function ensure_vp()
					if vp then return end
					vp = library:create("ViewportFrame", {
						Parent = well_body,
						Name = "",
						Active = true,
						Visible = false,
						...
						LightDirection = vec3(-0.6, -1, -0.4)
					})
					library:connection(vp.InputBegan, function(input)
						if input.UserInputType == Enum.UserInputType.MouseButton1
							or input.UserInputType == Enum.UserInputType.Touch then
							vp_drag = true; vp_dragx = input.Position.X; vp_dragrot = vp_rot
							vp_dragy = input.Position.Y; vp_dragpitch = vp_pitch
						end
					end)
					library:connection(uis.InputChanged, function(input)
						if vp_drag and (input.UserInputType == Enum.UserInputType.MouseMovement
							or input.UserInputType == Enum.UserInputType.Touch) then
							vp_rot = vp_dragrot + (input.Position.X - vp_dragx) * 0.6
							vp_pitch = math.clamp(vp_dragpitch + (input.Position.Y - vp_dragy) * 0.6, -85, 85)
						end
					end)
					library:connection(uis.InputEnded, function(input)
						if input.UserInputType == Enum.UserInputType.MouseButton1
							or input.UserInputType == Enum.UserInputType.Touch then vp_drag = false end
					end)
					library:connection(run.RenderStepped, function(dt)
						if not (vp.Visible and vp_model and vp_base) then return end
						if not vp_sg then vp_sg = vp:FindFirstAncestorWhichIsA("ScreenGui") end
						if vp_sg and not vp_sg.Enabled then return end
						if not vp_drag then vp_rot = vp_rot + (dt or 1 / 60) * 35 end
						pcall(function() vp_model:PivotTo(angle(rad(vp_pitch), 0, 0) * angle(0, rad(vp_rot), 0) * vp_base) end)
					end)
				end

				local function clear_model()
					if vp_world then pcall(function() vp_world:Destroy() end) end
					vp_world, vp_model, vp_base = nil, nil, nil
				end
				...
				function cfg.set_model(model, options)
					options = options or {}
					ensure_vp()
					clear_model()
					if not model then
					...
					local ok = pcall(function()
						...
						if options.dress then pcall(options.dress, clone) end
						for _, d in ipairs(clone:GetDescendants()) do
						...
						elseif clone:IsA("BasePart") then
							...
							world:Destroy(); error("not a model")
						end
						if size.Magnitude < 0.1 then size = vec3(2, 2, 2) end
						clone:PivotTo(clone:GetPivot() - centre)

						local cam = Instance.new("Camera")
						cam.FieldOfView = 40
						cam.Parent = vp
						vp.CurrentCamera = cam
						local dist = (size.Magnitude * 0.5) / tan(rad(cam.FieldOfView * 0.5)) * 1.25
						cam.CFrame = CFrame.lookAt(vec3(1, 0.45, 1).Unit * dist, vec3())

						vp_world, vp_model, vp_base = world, clone, clone:GetPivot()
					end)
					if not ok then
						vp.Visible = false; preview.Visible = true; clear_model(); return
					end
				...
				function cfg.set_preview_visible(v)
					well.Visible = v and true or false
					well.Size = dim2(1, 0, 0, v and cfg.height or 0)
				end
			...
			function library:slider(options)
				...
				function cfg.set(value)
					...
					slidertext.Text = (cfg.labels and cfg.labels[cfg.value]) or (tostring(cfg.value) .. cfg.suffix .. "/" .. tostring(cfg.max) .. cfg.suffix)
			...
			function library:plus(options)
				...
				do
					...
					local function fit_height()
						if options and options.height then return end
				...
				function cfg.set_visible(bool)
					...
					if bool then
						...
						library:track_popup(window, plus_button, true)
					elseif library.tracked_popup and library.tracked_popup.frame == window then
						library.tracked_popup = nil
					end
				end
			...
			function library:colorpicker(options)
				...
				local cfg = {
					...
					tooltip = options.tooltip or nil,
					use_accent = options.use_accent or false,
				}
				...
				function cfg.set_visible(bool)
					...
					if bool then
						...
						library.current_element_open = cfg
						library:track_popup(colorpicker_holder, colorpicker_button)
					else
						if library.tracked_popup and library.tracked_popup.frame == colorpicker_holder then
							library.tracked_popup = nil
						end
					end
				end
				...
				function cfg.set(color, alpha)
					...
					visualize.BackgroundTransparency = 1 - a
				...
				library:create("UIPadding", {
					...
					function cfg.set_visible(bool)
						...
						if bool then
							...
							library.current_element_open = cfg
							library:track_popup(keybind_selector, element_outline)
						elseif library.tracked_popup and library.tracked_popup.frame == keybind_selector then
							library.tracked_popup = nil
						end
					end
			...
			function library:dropdown(options)
				...
				function cfg.set_visible(bool)
					...
					if bool then
						...
						fit_holder()
						library:track_popup(dropdown_holder, dropdown)
					elseif library.tracked_popup and library.tracked_popup.frame == dropdown_holder then
						library.tracked_popup = nil
					end
				end
			...
			function library:model_grid(options)
				...
				local function paint(cell)
					...
					cell.wash.Visible = on
					cell.wash.BackgroundColor3 = themes.preset.accent
					cell.pip.Visible = on
				...
				local TIER_RANK = {
					...
					rare = 3, uncommon = 2, common = 1, norarity = 0,
				}
				local entries = {}
				for _, child in ipairs(cfg.source:GetChildren()) do
					...
					if keep then
						local rcol, rname, rank
						if cfg.rarity then
							local okr, c, n, r = pcall(cfg.rarity, child.Name)
							if okr and typeof(c) == "Color3" then
								rcol, rname = c, n
								rank = tonumber(r) or TIER_RANK[(tostring(n):lower():gsub("[^%a]", ""))]
							end
						end
						entries[#entries + 1] = {
							child = child, rarity = rcol, tier = rname, rank = rank or -1
						}
					end
				end
				...
				for _, entry in ipairs(entries) do
					...
					local body = library:create("UIGradient", {
						Parent = btn,
						Rotation = 90,
						Color = rgbseq{rgbkey(0, rgb(41, 41, 55)), rgbkey(1, rgb(35, 35, 47))}
					}) library:apply_theme(body, "contrast", "Color")
					local stroke = library:create("UIStroke", {
						Parent = btn,
						Color = themes.preset.outline,
						Thickness = 1,
					...
					local wash = library:create("Frame", {
						...
						BackgroundColor3 = themes.preset.accent
					}) library:apply_theme(wash, "accent", "BackgroundColor3")
					library:create("UICorner", { Parent = wash, CornerRadius = UDim.new(0, 3) })
			...
			function library:playerlist(options)
				...
				function library.prioritize(text)
					...
					if library.selected_is_local then
						return
					end
	...
	do
		local genv = (getgenv and getgenv()) or _G
		genv.DELTA_UILIB = { library, themes }
	end
